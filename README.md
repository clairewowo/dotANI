...
_dotANI_ first samples the kmer set and then estimate intersection via dotHash. Then the kmer hashes are encoded into hyperdimensional vectors (HVs) using HDC encoding to obtain better tradeoff of ANI estimation quality, sketch size, and computation speed. To obtains the cardinality of genomes, we use UltraLogLog. The sketch size generated by _dotANI_ is 1.8 to 2.7× smaller than _Mash_ and _Dashing 2_. ANI estimation in _dotANI_ can be realized using highly vectorized vector multiplication. _dotANI_'s database search speed for large-scale datasets is up to 4.3x faster than _Dashing 2_.
